...
\usepackage[letterpaper, margin=2cm, includeheadfoot]{geometry}
\usepackage{setspace}
\usepackage{graphicx}
\graphicspath{{images/}}
...
\usepackage{lipsum}
\usepackage{multirow}
% --- DEFINICIONES DE DATOS PARA LA PORTADA ---
...
% --- HYPERREF (casi siempre el último) ---
\usepackage{hyperref}
...
\title{
	\vspace{2cm}\huge{\MakeUppercase{\textbf{\TITULO}}}\\
...
\date{\vspace{3cm}\realizacion}

% Se redefine el comando \maketitle para crear la portada personalizada
\makeatletter
\let\oldmaketitle\maketitle
\renewcommand{\maketitle}{%
	\thispagestyle{empty}%
	\AddToShipoutPicture*{\BackgroundPic}%
	{\color{white}\oldmaketitle}%
	\ClearShipoutPicture%
}
\makeatother
...
\newcommand\notaautor[1]{{\scriptsize\begin{flushright}\emph{(#1)}\end{flushright}}}
%%%Entornos de desarrollo
\newtheorem{ejemplo}{Ejemplo}
\newtheorem{cuadro1}{}
\newenvironment{cuadro}
  {\begin{mdframed}\begin{cuadro1}}
  {\end{cuadro1}\end{mdframed}}
...
% Tu definición de lenguaje HTML5 es muy extensa, la incluyo tal cual
...
\lstdefinelanguage[CustomSQL]{SQL}[]{SQL}{
    morecomment=[l]{//}, % Agrega soporte para // como comentario en SQL
}
...
\lstset{
	...
	keywordstyle=\ttfamily\bfseries\azul,
	commentstyle= \ttfamily\scriptsize\verde,
	stringstyle=\ttfamily\rojo,
...
% Comando para instrucciones visuales (El alumno debe borrar esto)
\newcommand{\instruccion}[1]{
    \begin{quote}
        \small\itshape\color{blue}
        \textbf{INSTRUCCIÓN:} #1
    \end{quote}
}

% Asignación dinámica de variables según la configuración
\ifprofesional
\def\subtitulo{Práctica Profesional (\horastotales)}
\def\asignatura{Práctica Profesional}
\else
\def\subtitulo{Práctica Intermedia (\horastotales)}
\def\asignatura{Práctica Intermedia}
\fi


\def\objetivospractica{
